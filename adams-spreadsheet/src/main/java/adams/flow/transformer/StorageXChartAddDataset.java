...
/**
 <!-- globalinfo-start -->
 * Generates datasets from the incoming spreadsheet and adds them to the specified XChart datasets in storage.<br>
...
public class StorageXChartAddDataset
  ...
  public String storageNameTipText() {
    return "The name of the storage item that represents the datasets to update.";
  }
  ...
  protected String doExecute() {
	...
	result = "Datasets not available from storage: " + m_StorageName;
      else
	datasets = (Datasets) getStorageHandler().getStorage().get(m_StorageName);

      if (result == null) {
	if (!datasets.getClass().equals(m_Dataset.generates())) {
	  result = "Datasets in storage (" + m_StorageName + ") are of type "
		     + Utils.classToString(datasets) + ", but dataset generator produces "
		     + Utils.classToString(m_Dataset.generates()) + "!";
	}
	else {
	  datasets.addAll(m_Dataset.generate(sheet));
	  getStorageHandler().getStorage().put(m_StorageName, datasets);
