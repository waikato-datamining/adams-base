...
public class Zero
  ...
  protected void doVisit(Row row, int colIndex) {
    if (row.hasCell(colIndex) && row.getCell(colIndex).isNumeric() && row.getCell(colIndex).toDouble() == 0)
      m_Count++;
  }
  ...
  protected SpreadSheet postVisit(SpreadSheet sheet, int colIndex) {
    ...
    row = result.addRow();
    row.addCell(0).setContent("Non-zero");
