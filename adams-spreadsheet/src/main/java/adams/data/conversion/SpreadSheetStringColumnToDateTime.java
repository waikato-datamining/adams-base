...
public class SpreadSheetStringColumnToDateTime
  ...
  public void defineOptions() {
    ...
    m_OptionManager.add(
	    "format", "format",
	    new DateFormatString("dd/MM/yyyy HH:mm:ss"));
  }
  ...
  public String columnTipText() {
    return "The column to convert to date/time; " + m_Column.getExample();
  }
  ...
  protected void convert(Cell cellOld, Cell cellNew) throws Exception {
    ...
    if ((content.trim().length() > 0) && (m_Formatter.check(content.trim())))
      cellNew.setContent(new DateTime(m_Formatter.parse(content.trim())));
    else
      cellNew.setMissing();
  }
}
