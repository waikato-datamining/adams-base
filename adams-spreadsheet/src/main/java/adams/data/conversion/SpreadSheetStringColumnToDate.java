...
/**
 * SpreadSheetStringColumnToDate.java
...
/**
 <!-- globalinfo-start -->
 * Converts the specified spreadsheet column from string to date, according to the provided format.<br>
...
public class SpreadSheetStringColumnToDate
  ...
  public String globalInfo() {
    return 
	"Converts the specified spreadsheet column from string to date, "
  ...
  public void defineOptions() {
    ...
    m_OptionManager.add(
	    "format", "format",
	    new DateFormatString("dd/MM/yyyy"));
  }
  ...
  protected void convert(Cell cellOld, Cell cellNew) throws Exception {
    ...
    if ((content.trim().length() > 0) && (m_Formatter.check(content.trim())))
      cellNew.setContent(m_Formatter.parse(content.trim()));
    else
      cellNew.setMissing();
  }
}
