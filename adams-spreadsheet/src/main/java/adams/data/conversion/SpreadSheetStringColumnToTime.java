...
/**
 <!-- globalinfo-start -->
 * Converts the specified spreadsheet column from string to time, according to the provided format.<br>
 ...
 * @version $Revision$
 */
public class SpreadSheetStringColumnToTime
  ...
  protected void convert(Cell cellOld, Cell cellNew) throws Exception {
    ...
    if ((content.trim().length() > 0) && (m_Formatter.check(content.trim())))
      cellNew.setContent(new Time(m_Formatter.parse(content).getTime()));
    else
      cellNew.setMissing();
  }
}
