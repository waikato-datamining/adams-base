...
/**
 <!-- globalinfo-start -->
 * Converts the specified spreadsheet column from string to long, according to the provided format.<br>
 ...
 * &nbsp;&nbsp;&nbsp;The column to convert to long; An index is a number starting with 1; the 
...
public class SpreadSheetStringColumnToLong
  ...
  protected void convert(Cell cellOld, Cell cellNew) throws Exception {
    ...
    if (content.trim().length() > 0)
      cellNew.setContent(m_Formatter.parse(content).longValue());
    else
      cellNew.setMissing();
  }
}
