...
/*
 * SpreadSheetRowLinePaintlet.java
...
public class SpreadSheetRowLinePaintlet
  ...
  public String globalInfo() {
    return "Paintlet for generating a line plot for row objects.";
  }
   ...
   */
  protected void drawData(Graphics g, SpreadSheetRow data, Color color, MarkerShape marker) {
    List<SpreadSheetRowPoint>	points;
    SpreadSheetRowPoint	curr;
    int			currX;
    int			currY;
    int			prevX;
    int			prevY;
    ...
    start = SpreadSheetRowUtils.findClosestX(points, (int) Math.floor(axisX.getMinimum()));
    if (start > 0)
      start--;
    end = SpreadSheetRowUtils.findClosestX(points, (int) Math.ceil(axisX.getMaximum()));
    if (end < data.size() - 1)
      end++;
