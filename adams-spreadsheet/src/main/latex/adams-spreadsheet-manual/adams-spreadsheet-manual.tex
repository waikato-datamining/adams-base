...
\begin{tight_itemize}
	...
	spreadsheet to select a subset of rows/columns, delete rows, update 
	cells.\footnote{adams-spreadsheet-query.flow}
...
\begin{tight_itemize}
	\item \textit{AggregateSpreadSheet} -- aggregates rows in a spreadsheet
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Tools}
The \textit{Spreadsheet file viewer} is a simple tool for viewing all spreadsheet
file formats that ADAMS supports. Figure \ref{spreadsheet-viewer} shows a
...
\begin{tight_itemize}
	\item \textit{length} -- the length of a string.
	\item \textit{lowercase} -- converts string to a lowercase one.
	...
	\item \textit{uppercase} -- converts string to an uppercase one.
\end{tight_itemize}
