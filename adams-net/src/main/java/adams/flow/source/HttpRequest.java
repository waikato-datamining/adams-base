...
public class HttpRequest
  ...
  implements HttpResponseVariableSupporter, HttpRedirectSupporter {
  ...
  protected String doExecute() {
    ...
    try {
      res = new Request(m_Method)
	      .url(m_URL.urlValue())
	      .headers(BaseKeyValuePair.toMap(m_Headers))
	      .formData(new FormData().add(BaseKeyValuePair.toMap(m_Parameters)))
	      .cookies(manager)
	      .allowRedirects(m_AllowRedirects)
	      .execute();
      cont = new HttpRequestResult(res.statusCode(), res.statusMessage(), res.text());
      if (!m_VariableStatusCode.isDefault())
