...
public class OvalMapOverlay
  ...
  public String globalInfo() {
    return "Paints an oval at the specified location.";
  }
  ...
  public String fillEnabledTipText() {
    return "If enabled, the oval gets filled.";
  }
  ...
  protected void doPaintOverlay(OpenStreetMapViewer viewer, Graphics g, int x, int y) {
    if (m_FillEnabled) {
      ...
      g.fillOval(x, y - m_Height, m_Width, m_Height);
    }

    if (m_BorderEnabled) {
      g.setColor(m_BorderColor);
      g.drawOval(x, y - m_Height, m_Width, m_Height);
    }
  }
}
