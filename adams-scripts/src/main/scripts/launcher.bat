...
%JCMD% %JAVA_OPTS% -classpath %CLASSPATH_PREFIX%;%CLASSPATH% -Dbasedir="%BASEDIR%" adams.core.management.Launcher -memory %MEMORY% -javaagent %AGENT% %JVM% %CPA% %PRIORITY% %ENVVARS% %COLLAPSE% -jvm --add-opens=java.desktop/sun.awt.shell=ALL-UNNAMED -jvm --add-exports=java.base/jdk.internal.misc=ALL-UNNAMED -jvm --add-exports=java.desktop/sun.awt.image=ALL-UNNAMED -main %MAIN% -doc-dir "%BASEDIR%\docs" %CMD_LINE_ARGS%
