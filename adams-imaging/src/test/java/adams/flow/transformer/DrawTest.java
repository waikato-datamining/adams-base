...
public class DrawTest
  ...
  public AbstractActor getActor() {
    ...
    try {
      ...
      tmp59[0] = tmp60;
      adams.flow.transformer.Draw tmp62 = new adams.flow.transformer.Draw();
      adams.flow.transformer.draw.Pixel tmp62a = new adams.flow.transformer.draw.Pixel();
      argOption = (AbstractArgumentOption) tmp62a.getOptionManager().findByProperty("X");
      argOption.setVariable("@{X}");

      argOption = (AbstractArgumentOption) tmp62a.getOptionManager().findByProperty("Y");
      argOption.setVariable("@{Y}");

      argOption = (AbstractArgumentOption) tmp62a.getOptionManager().findByProperty("RGBA");
      argOption.setVariable("@{RGBA}");

      tmp62.setOperation(tmp62a);
      tmp59[1] = tmp62;
