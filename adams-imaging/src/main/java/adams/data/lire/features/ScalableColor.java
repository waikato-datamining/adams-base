...
/**
 <!-- globalinfo-start -->
 * Generates features using net.semanticmetadata.lire.imageanalysis.features.global.ScalableColor.<br>
...
public class ScalableColor
  ...
  /** for serialization. */
  private static final long serialVersionUID = 8690893367113806689L;
  ...
  public String globalInfo() {
    return
        "Generates features using " + net.semanticmetadata.lire.imageanalysis.features.global.ScalableColor.class.getName() + ".\n"
  ...
  public HeaderDefinition createHeader(BufferedImageContainer img) {
    ...
    double[]			histo;
    int				i;
    net.semanticmetadata.lire.imageanalysis.features.global.ScalableColor	features;

    image    = BufferedImageHelper.convert(img.getImage(), BufferedImage.TYPE_3BYTE_BGR);
    features = new net.semanticmetadata.lire.imageanalysis.features.global.ScalableColor();
    features.extract(image);
    histo    = features.getFeatureVector();
    result   = new HeaderDefinition();
    for (i = 0; i < histo.length; i++)
      result.add("ScalableColor-" + (i+1), DataType.NUMERIC);
