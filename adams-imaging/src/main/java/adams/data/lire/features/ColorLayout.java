...
/**
 <!-- globalinfo-start -->
 * Generates features using net.semanticmetadata.lire.imageanalysis.features.global.ColorLayout.<br>
...
public class ColorLayout
  ...
  /** for serialization. */
  private static final long serialVersionUID = 4505596265143958208L;
  ...
  public String globalInfo() {
    return
        "Generates features using " + net.semanticmetadata.lire.imageanalysis.features.global.ColorLayout.class.getName() + ".\n"
  ...
  public HeaderDefinition createHeader(BufferedImageContainer img) {
    ...
    double[]			histo;
    int				i;
    net.semanticmetadata.lire.imageanalysis.features.global.ColorLayout	features;

    image    = BufferedImageHelper.convert(img.getImage(), BufferedImage.TYPE_3BYTE_BGR);
    features = new net.semanticmetadata.lire.imageanalysis.features.global.ColorLayout();
    features.extract(image);
    histo    = features.getFeatureVector();
    result   = new HeaderDefinition();
    for (i = 0; i < histo.length; i++)
      result.add("ColorLayout-" + (i+1), DataType.NUMERIC);
