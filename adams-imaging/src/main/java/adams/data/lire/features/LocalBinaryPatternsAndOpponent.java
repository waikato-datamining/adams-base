...
/*
 * LocalBinaryPatternsAndOpponent.java
...
/**
 <!-- globalinfo-start -->
 * Generates features using net.semanticmetadata.lire.imageanalysis.features.global.joint.LocalBinaryPatternsAndOpponent.<br>
...
public class LocalBinaryPatternsAndOpponent
  ...
  public String globalInfo() {
    return
        "Generates features using " + net.semanticmetadata.lire.imageanalysis.features.global.joint.LocalBinaryPatternsAndOpponent.class.getName() + ".\n"
  ...
  public HeaderDefinition createHeader(BufferedImageContainer img) {
    ...
    double[]			histo;
    int				i;
    net.semanticmetadata.lire.imageanalysis.features.global.joint.LocalBinaryPatternsAndOpponent	features;

    image    = BufferedImageHelper.convert(img.getImage(), BufferedImage.TYPE_3BYTE_BGR);
    features = new net.semanticmetadata.lire.imageanalysis.features.global.joint.LocalBinaryPatternsAndOpponent();
    features.extract(image);
    histo    = features.getFeatureVector();

    result   = new HeaderDefinition();
    for (i = 0; i < histo.length; i++)
      result.add("LocalBinaryPatternsAndOpponent-" + (i+1), DataType.NUMERIC);
