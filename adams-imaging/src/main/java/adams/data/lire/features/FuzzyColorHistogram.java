...
/**
 <!-- globalinfo-start -->
 * Generates features using net.semanticmetadata.lire.imageanalysis.features.global.FuzzyColorHistogram.<br>
...
public class FuzzyColorHistogram
  ...
  /** for serialization. */
  private static final long serialVersionUID = -748267132930230243L;
  ...
  public String globalInfo() {
    return
        "Generates features using " + net.semanticmetadata.lire.imageanalysis.features.global.FuzzyColorHistogram.class.getName() + ".\n"
  ...
  public HeaderDefinition createHeader(BufferedImageContainer img) {
    ...
    double[]			histo;
    int				i;
    net.semanticmetadata.lire.imageanalysis.features.global.FuzzyColorHistogram		features;

    image    = BufferedImageHelper.convert(img.getImage(), BufferedImage.TYPE_3BYTE_BGR);
    features = new net.semanticmetadata.lire.imageanalysis.features.global.FuzzyColorHistogram();
    features.extract(image);
    histo    = features.getFeatureVector();
    result   = new HeaderDefinition();
    for (i = 0; i < histo.length; i++)
      result.add("FuzzyColorHistogram-" + (i+1), DataType.NUMERIC);
