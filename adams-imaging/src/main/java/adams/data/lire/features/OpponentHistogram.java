...
/**
 <!-- globalinfo-start -->
 * Generates features using net.semanticmetadata.lire.imageanalysis.OpponentHistogram.<br/>
...
public class OpponentHistogram
  ...
  public String globalInfo() {
    return
        "Generates features using " + net.semanticmetadata.lire.imageanalysis.OpponentHistogram.class.getName() + ".\n"
  ...
  public Instances createHeader(BufferedImageContainer img) {
    ...
    net.semanticmetadata.lire.imageanalysis.OpponentHistogram		features;
    BufferedImage		image;
    double[]			histo;
    int				i;

    image    = BufferedImageHelper.convert(img.getImage(), BufferedImage.TYPE_3BYTE_BGR);
    features = new net.semanticmetadata.lire.imageanalysis.OpponentHistogram();
    features.extract(image);
    histo    = features.getDoubleHistogram();
    
    atts = new ArrayList<Attribute>();
    for (i = 0; i < histo.length; i++)
      atts.add(new Attribute("OpponentHistogram-" + (i+1)));
