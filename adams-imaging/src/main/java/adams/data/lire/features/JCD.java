...
/**
 <!-- globalinfo-start -->
 * Generates features using net.semanticmetadata.lire.imageanalysis.features.global.JCD.<br>
...
public class JCD
  ...
  public String globalInfo() {
    return
        "Generates features using " + net.semanticmetadata.lire.imageanalysis.features.global.JCD.class.getName() + ".\n"
  ...
  public HeaderDefinition createHeader(BufferedImageContainer img) {
    ...
    double[]			histo;
    int				i;
    net.semanticmetadata.lire.imageanalysis.features.global.JCD	features;

    image    = BufferedImageHelper.convert(img.getImage(), BufferedImage.TYPE_3BYTE_BGR);
    features = new net.semanticmetadata.lire.imageanalysis.features.global.JCD();
    features.extract(image);
    histo    = features.getFeatureVector();
    result   = new HeaderDefinition();
    for (i = 0; i < histo.length; i++)
      result.add("JCD-" + (i+1), DataType.NUMERIC);
