 ...
 */
public class DeepLabCutCSVHandler
  ...
  public class CombinedPanel
    ...
    protected void initGUI() {
      ...
      m_ChooserAlternative = new FileChooserPanel();
      m_ChooserAlternative.setEnabled(false);
      m_ChooserAlternative.addChangeListener((ChangeEvent e) -> updateReport());
      m_ChooserAlternative.setFileFilter(new ExtensionFileFilter("DeepLabCut CSV", "csv"));
  ...
  public String prefixTipText() {
    return "The prefix of fields in the annotations to identify as point location, eg 'Point.'.";
  }
  ...
  public String colorTipText() {
    return "The color to use for the points.";
  }
  ...
  protected Report loadReport(CombinedPanel panel, File file) {
    Report 		result;
    File		reportFile;
    DeepLabCutCSVReader reportReader;
    List<Report> 	reports;

    result = null;
    if (panel.getUseAlternativeLocation())
      reportFile = panel.getAlternativeLocation();
    else
      reportFile = FileUtils.replaceExtension(file, ".csv");;
    if (reportFile.exists() && reportFile.isFile()) {
      reportReader = new DeepLabCutCSVReader();
      reportReader.setFileRegExp(new BaseRegExp(".*" + file.getName()));
