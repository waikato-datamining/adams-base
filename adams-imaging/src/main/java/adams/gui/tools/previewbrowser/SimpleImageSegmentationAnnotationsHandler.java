...
public class SimpleImageSegmentationAnnotationsHandler
  ...
  protected PreviewPanel createPreview(File file) {
    ...
    if (annotationFile == null)
      return new NoPreviewAvailablePanel("Failed to locate annotation file for:\n" + file.getAbsolutePath());
    ...
    if (colorized.length != 1)
      return new NoPreviewAvailablePanel("Failed to colorized image:\n" + annotationFile.getAbsolutePath());
    ...
    // combine images
    dest = new BufferedImage(baseImg.getWidth(), baseImg.getHeight(), BufferedImage.TYPE_INT_ARGB);
    g2 = dest.createGraphics();
    g2.drawImage(baseImg, 0, 0, null);
    ac = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) m_Alpha / 255);
    g2.setComposite(ac);
