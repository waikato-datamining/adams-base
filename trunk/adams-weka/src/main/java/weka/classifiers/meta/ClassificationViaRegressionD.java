...
public class ClassificationViaRegressionD
  ...
  protected MakeIndicator[] m_ClassFilters;
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
    ...
    for (i = 0; i < inst.numClasses(); i++) {
      ...
      if (probs[i] > 1)
        probs[i] = 1;
      if (probs[i] < 0 || Double.isNaN(probs[i]))
	probs[i] = 0;
      sum += probs[i];
    }
   ...
   *
   * @param args	the options for the learner
   */
  public static void main(String[] args){
    runClassifier(new ClassificationViaRegressionD(), args);
  }
}
