...
/**
 <!-- globalinfo-start -->
 * Adds the elements of a map to the matlab file object using the names from the map.<br>
  ...
  protected String doExecute() {
    ...
    if (!errors.isEmpty()) {
      ...
      if (!(obj instanceof Map))
        result = "Retrieved object is not a map: " + Utils.classToString(obj);
    }
    ...
    if (result == null) {
      ...
      try {
        for (Object key: map.keySet()) {
          if (isLoggingEnabled())
            getLogger().info("Adding value for key: " + key);
          value  = map.get(key);
	    ...
	    mat5.addArray(keyStr, (Array) value);
            ...
            getLogger().warning(
              "Value '" + key + "' is not a " + Utils.classToString(Array.class)
		+ " but: " + Utils.classToString(value));
	  }
	}
	m_OutputToken = new Token(mat5);
      }
      catch (Exception e) {
        result = handleException("Failed to add entries from map!", e);
      }
    }
