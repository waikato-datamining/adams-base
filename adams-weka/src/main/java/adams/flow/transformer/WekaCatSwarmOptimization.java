...
public class WekaCatSwarmOptimization
  ...
  implements CatSwarmOptimizationFitnessChangeListener, CallableActorUser, OptionalCallableActor,
  ...
  public String globalInfo() {
    return
        "Applies the cat swarm optimization (CSO) algorithm to the incoming dataset.\n"
  ...
  /**
   * Sets the swarm algorithm to apply to the dataset.
   *
  ...
  /**
   * Returns the swarm algorithm to apply to the dataset.
   *
  ...
  protected String doExecute() {
    String				result;
    Instances				data;
    WekaCatSwarmOptimizationContainer	cont;

    result = null;

    data              = (Instances) m_InputToken.getPayload();
    cont              = null;
    m_ActualAlgorithm = (ClassifierBasedCatSwarmOptimization) OptionUtils.shallowCopy(m_Algorithm);
    m_ActualAlgorithm.addFitnessChangeListener(this);
    if (m_ActualAlgorithm instanceof AbstractJobBasedCatSwarmOptimization) {
      ((AbstractJobBasedCatSwarmOptimization) m_ActualAlgorithm).setJobRunnerSetup(m_JobRunnerSetup);
      ((AbstractJobBasedCatSwarmOptimization) m_ActualAlgorithm).setFlowContext(this);
    }
    try {
	...
	cont = new WekaCatSwarmOptimizationContainer(
	  m_ActualAlgorithm.getBestSetup(),
      ...
      result = handleException("Failed to run swarm algorithm!", e);
    }
  ...
  public void fitnessChanged(CatSwarmOptimizationFitnessChangeEvent e) {
	...
	cont = new WekaCatSwarmOptimizationContainer(
	  ...
	  ((ClassifierBasedCatSwarmOptimization) e.getCatSwarmOptimization()).getMeasure(),
	  e.getFitness());
