...
public abstract class AbstractProcessSelectedRows
   ...
   */
  protected abstract boolean doProcessSelectedRows(TableState state);
  ...
  public boolean processSelectedRows(TableState state) {
    ...
    result = (error == null);
    if (result)
      result = doProcessSelectedRows(state);
    else
      GUIHelper.showErrorMessage(state.table, "Failed to process rows (0-based indices): " + Utils.arrayToString(state.actRows) + "\n" + error);
