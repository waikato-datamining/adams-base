...
public class WekaInvestigator
   ...
   */
  public WekaInvestigator(AbstractApplicationFrame owner) {
    super(owner);
  }
  ...
  @Override
  public void launch() {
    InvestigatorManagerPanel investigator = new InvestigatorManagerPanel();
    createChildFrame(investigator, GUIHelper.makeWider(GUIHelper.getDefaultLargeDialogDimension()));
    if (m_Parameters.length > 0) {
      for (int i = 0; i < m_Parameters.length; i++)
        investigator.getCurrentPanel().openFile(new PlaceholderFile(m_Parameters[i]));
    }
  }
