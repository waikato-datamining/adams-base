...
import weka.filters.unsupervised.attribute.multiplicativescattercorrection.RangeBased;
...
public class MultiplicativeScatterCorrection
  ...
  protected AbstractMultiplicativeScatterCorrection m_Correction = getDefaultCorrection();
  ...
  public String globalInfo() {
    return "Performs Multiplicative Scatter Correction, using the specified correction scheme.";
  }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    setCorrection((AbstractMultiplicativeScatterCorrection) WekaOptionUtils.parse(options, CORRECTION, (adams.core.option.OptionHandler) getDefaultCorrection()));
   ...
   * @return		the default
   */
  protected AbstractMultiplicativeScatterCorrection getDefaultCorrection() {
  ...
  protected Instances process(Instances instances) throws Exception {
    ...
    // calculate average
    if (m_Average == null) {
      m_Average = new double[m_AttIndices.size()];
      for (n = 0; n < instances.numInstances(); n++) {
        inst = instances.instance(n);
        for (i = 0; i < m_AttIndices.size(); i++)
          m_Average[i] += inst.value(m_AttIndices.get(i));
      }
      for (i = 0; i < m_AttIndices.size(); i++)
	m_Average[i] /= instances.numInstances();
      if (getDebug())
	System.err.println(getClass().getName() + ": Average: " + Utils.arrayToString(m_Average));
    }
	...
	data[i] = inst.value(m_AttIndices.get(i));
      data   = m_Correction.correct(m_WaveNo, m_Average, data);
  ...
  public static void main(String [] args) {
    ...
    runFilter(new MultiplicativeScatterCorrection(), args);
  }
}
