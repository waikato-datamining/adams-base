...
public class AbstainAverageWithClassifierWeights
  ...
  public String [] getOptions() {
    ...
    result.add("-F");
    result.add("" + getClassifierWeights());
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    parseWeights(getClassifierWeights());
  ...
  protected void parseWeights(String input){
    m_ClassifierWeights = new double[m_Classifiers.length];
    // initialise weights
    for (int i=0;i<m_ClassifierWeights.length;i++){
      m_ClassifierWeights[i]=1.0;
    }
    if (input == null || input.equals("")){
      return;
    }
    String[] entries=input.split(",");
    if (entries.length!=m_ClassifierWeights.length){
      System.err.println("Weights no formatted correctly");
      return;
    }
    for (int i=0;i<m_ClassifierWeights.length;i++){
      m_ClassifierWeights[i]=Double.parseDouble(entries[i]);
    }
  }
  ...
  protected double classificationNumericAverage(Instance instance, boolean abstain) throws Exception{
    ...
    for (int i = 0; i < m_Classifiers.length; i++) {
      ...
      sum +=(pred*m_ClassifierWeights[i]);
      weightsum += m_ClassifierWeights[i];
    }
  ...
  public static void main(String [] argv) {
    runClassifier(new AbstainAverageWithClassifierWeights(), argv);
  }
  ...
  public String getClassifierWeights(){
    return(m_ClassifierWeights_string);
  }
  public void  setClassifierWeights(String s){
    m_ClassifierWeights_string=s;
  }
  /**
   * Returns the tip text for this property
   * @return tip text for this property suitable for
