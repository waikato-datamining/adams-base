...
/**
 * Submenu for adding a subflow generated from a template.
 * 
...
public class AddFromTemplate
  ...
  protected JMenuItem getMenuItem(final StateContainer state) {
    ...
    result = new BaseMenu("Template");
    result.setIcon(GUIHelper.getEmptyIcon());
    result.setEnabled(state.isSingleSel && state.editable);
    new AddFromTemplateBeneath().add(state, result);
    result.addSeparator();
    new AddFromTemplateHere().add(state, result);
    new AddFromTemplateAfter().add(state, result);
    new LastTemplate().add(state, result);
