...
public class MostCommon
  ...
  public final static String FILENAME = "FlowMostCommonEnclosingActors.csv";
  ...
  protected static String save() {
    ...
    row.addCell("handler").setContentAsString("Handler");
    ...
    if (sheet.getRowCount() > 0) {
      ...
      if (!writer.write(sheet, getFilename()))
        return "Failed to update most common actors used for enclosing: " + getFilename();
    }
  ...
  protected void doRecord(Tree tree, TreePath[] paths, ActorHandler handler) {
    initCounter();
    m_Counter.next(handler.getClass().getName());
    saveCounter();
  }
