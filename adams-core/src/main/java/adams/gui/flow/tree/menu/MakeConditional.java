...
public class MakeConditional
  ...
  protected JMenuItem getMenuItem(final StateContainer state) {
    ...
    result = new JMenuItem("Make conditional...");
  ...
  protected AbstractTreeShortcut newShortcut() {
	...
	return 
	       state.editable 
	    && state.isSingleSel 
	    && (state.tree.getOwner() != null) 
	    && (state.selNode.getActor() instanceof ActorWithConditionalEquivalent);
      }
      @Override
      protected void doExecute(StateContainer state) {
	state.tree.makeConditional(state.selPath);
      }
    };
  }
}
