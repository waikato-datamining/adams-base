...
/**
 * For adding a Breakpoint beneath the current one.
 * 
...
public class AddBreakpointBeneath
  ...
  protected JMenuItem getMenuItem(final StateContainer state) {
    ...
    result = new JMenuItem("Break beneath...");
  ...
  protected AbstractTreeShortcut newShortcut() {
	...
	return state.editable && state.isSingleSel && state.isMutable;
      }
      @Override
      protected void doExecute(StateContainer state) {
	state.tree.addActor(state.selPath, new Breakpoint(), InsertPosition.BENEATH);
      }
    };
  }
}
