...
public class LastTemplate
  ...
  protected JMenuItem getMenuItem(final StateContainer state) {
    ...
    if (state.lastTemplate != null) {
      result = new JMenuItem("Last template...");
  ...
  protected AbstractTreeShortcut newShortcut() {
	...
	return (state.lastTemplate != null) && state.editable && ((state.lastTemplateInsertPosition == InsertPosition.BENEATH) && state.isMutable) || state.isParentMutable;
      }
      @Override
      protected void doExecute(StateContainer state) {
	state.tree.addFromTemplate(state.selPath, state.lastTemplate, state.lastTemplateInsertPosition);
      }
    };
  }
}
