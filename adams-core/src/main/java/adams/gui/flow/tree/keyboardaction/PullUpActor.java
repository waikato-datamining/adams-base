...
public class PullUpActor
  ...
  protected BaseShortcut getDefaultShortcut() {
    return new BaseShortcut("shift pressed F4");
  }
  ...
  protected String check(StateContainer state) {
	...
	result = "Exactly one actor must be selected";
      else if (!state.isParentMutable)
        result = "Parent is not a mutable actor handler";
      else if (!(state.selNode.getActor() instanceof MutableActorHandler))
        result = "Selected actor is not a mutable actor handler";
    }
  ...
  protected String doExecute(StateContainer state) {
    state.tree.getOperations().pullUpActors(state.selPath);
    return null;
  }
}
