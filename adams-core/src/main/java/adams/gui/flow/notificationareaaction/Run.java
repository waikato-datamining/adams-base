...
public class Run
  ...
  public Run() {
    super();
    setName("Run");
    setIcon(GUIHelper.getIcon("run.gif"));
  }
  ...
  protected void doActionPerformed(ActionEvent e) {
    if (getOwner().getOwner() instanceof FlowPanel)
      ((FlowPanel) getOwner().getOwner()).run();
    else if (getOwner().getOwner() instanceof FlowRunnerPanel)
      ((FlowRunnerPanel) getOwner().getOwner()).run();
  ...
  public void update() {
    setEnabled(
      (getOwner().getOwner() instanceof FlowPanel)
        || (getOwner().getOwner() instanceof FlowRunnerPanel) );
  }
}
