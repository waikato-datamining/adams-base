...
public class CallableActorsHandler
  ...
  public String globalInfo() {
    return "Forwards the responses to the specified (optional) callable actor.";
  }
  ...
  public void responseSuccessful(RemoteCommand cmd) {
    ...
    if (isLoggingEnabled())
      getLogger().fine("Successful response: " + OptionUtils.getCommandLine(cmd) + "\n" + cmd);

    error = send(RemoteCommandContainer.EVENT_RESPONSESUCCESSFUL, cmd, null);
    if (error != null)
  ...
  public void responseFailed(RemoteCommand cmd, String msg) {
    ...
    if (isLoggingEnabled())
      getLogger().fine("Failed response: " + OptionUtils.getCommandLine(cmd) + "\nMessage: " + msg + "\n" + cmd);

    error = send(RemoteCommandContainer.EVENT_RESPONSEFAILED, cmd, msg);
    if (error != null)
      getLogger().severe(error);
  }
}
