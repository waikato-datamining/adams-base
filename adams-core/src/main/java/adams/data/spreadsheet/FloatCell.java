...
public class FloatCell
  ...
  public void assign(Cell cell) {
	...
	default:
	  if (cell instanceof FloatCell)
	    m_Content = ((FloatCell) cell).m_Content;
	  else
   ...
   * @see		SpreadSheet#getTimeFormat()
   */
  protected boolean checkFloat(String s) {
    return Utils.isFloat(s, getSpreadSheet().getLocale());
  }
  ...
  public Cell parseContent(String value) {
    if (checkBoolean(value)) {
      ...
      try {
	setContent(Utils.toFloat(value, getSpreadSheet().getLocale()));
      }
      catch (Exception e) {
  ...
  /**
   * Returns the content as long, if possible. First, a Float object is
  ...
  public Long toLong() {
    ...
    if (m_ContentType == ContentType.LONG)
      ...
      return new Float(getContent()).longValue();
    return null;
  }
  ...
  protected void calculateIfRequired() {
    if (isFormula() && Float.isNaN(m_Content))
      calculate();
  }
}