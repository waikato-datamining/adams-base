...
/*
 * ScottsNormalReferenceRuleBinning.java
 ...
 */
public class ScottsNormalReferenceRuleBinning<T>
  ...
  public String globalInfo() {
    return "Scott's normal reference rule is optimal for random samples of "
      + "normally distributed data, in the sense that it minimizes the "
  ...
  protected List<Bin<T>> doGenerateBins(List<Binnable<T>> objects) {
    ...
    stdev  = StatUtils.stddev(values, true);
    if (isLoggingEnabled())
      getLogger().info("stdev=" + stdev);
    return doGenerateBins(objects, (3.5 * stdev) / Math.pow(objects.size(), 1./3.));
  }
}
