...
public class MultiMonitor
  ...
  protected DirChangeMonitor[] m_Monitors;
   ...
   */
  public void setMonitors(DirChangeMonitor[] value) {
    m_Monitors = value;
    reset();
  }
   ...
   * @return  		the monitors
   */
  public DirChangeMonitor[] getMonitors() {
    return m_Monitors;
  }
  ...
  public String getQuickInfo() {
    ...
    result += QuickInfoHelper.toString(this, "monitors", m_Monitors, ", monitors: ");
  ...
  protected String doInitialize(File dir) {
    ...
    if (m_Monitors.length == 0) {
      ...
      for (i = 0; i < m_Monitors.length; i++) {
        result = m_Monitors[i].initialize(dir);
        if (result != null) {
  ...
  protected boolean checkChange(File dir) {
	...
	case AND:
	  result = m_Monitors[i].hasChanged(dir);
	  if (!result)
	    break;
	  break;
  ...
  protected String doUpdate(File dir) {
    ...
    if (m_Monitors.length == 0) {
      ...
      for (i = 0; i < m_Monitors.length; i++) {
        result = m_Monitors[i].update(dir);
        if (result != null) {
	  result = "Monitor #" + (i + 1) + ": " + result;
	  break;
	}
