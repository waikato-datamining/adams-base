...
global actors and variable support.
...
\begin{tight_itemize}
	...
	\item \textit{ClearGlobalDisplay} -- Can be used to clear globally defined
	graphical actors, e.g., a \textit{SequencePlotter}. See \ref{global_actors} for
	more information on global actors.
	...
	\item \textit{GlobalActorScreenshot} -- For taking screenshots of a globally
	defined (graphical) actor, whenever a token passes through this control actor.
	See \ref{global_actors} for	more information on global actors.
	...
	\item \textit{LocalScope} -- Provides ``local'' variables and internal
	...
	executed to ensure that flow execution continues .
...
\begin{tight_itemize}
	...
	a binary file and outputs the bytes one-by-one.
...
remedy for this: \textbf{global actors}. With this mechanism, multiple data
streams can once again be channeled into a single actor again, simulating a
n-to-1 relationship. And here is what to do:
\begin{tight_itemize}
	\item Add the \textit{GlobalActors} standalone actor at the start of the flow.
	\item Add the actor that you want to channel the data into below the
	\textit{GlobalActors} actor that you just added. In our example, this is the
	\textit{Display} actor.
	\item Replace each occurence of the actor that you just added below the
	\textit{GlobalActors} actor with a \textit{GlobalSink} sink actor. Enter as
	value for the parameter \textit{globalName} the name of the actor that you
	added below the \textit{GlobalActors} actor. In this example this is simply
	\textit{Display}.
\end{tight_itemize}
Figure \ref{floweditor-helloworld-global_display} shows the modified flow
\footnote{adams-core-hello\_world7.flow}, using a single global \textit{Display}
actor and multiple \textit{GlobalSink} actors.
\begin{figure}[htb]
  \centering
  \includegraphics[width=10.0cm]{images/floweditor-helloworld-global_display.png}
  \caption{Outputting parallel processed strings in a single global
  \textit{Display} actor.}
  \label{floweditor-helloworld-global_display}
\end{figure}
In addition to the \textit{GlobalSink} actor, there are also the
\textit{GlobalSource} actor (for using the same source multiple times in a flow)
and the \textit{GlobalTransformer} actor (for instance for using the same
preprocessing multiple times). They are used in the same fashion as the
\textit{GlobalSink} actor that we just introduced.
...
referenced from within the flow using \textit{GlobalSink} actors.
...
\heading{Using global actor as variables}
...
by attaching global actors to options. Attaching a global actor works just like
attaching a simple variable, only the naming convention is different:
\begin{verbatim}
  @{global:<global-actor-name>}
\end{verbatim}
The \texttt{global:} prefix tells ADAMS that the following name is referencing a
global actor. It then locates the actor and executes it to obtain the value for
using with the option in question.

This mechanism has mainly three caveats:
\begin{tight_itemize}
	\item The global actor (or sequence of actors) gets executed whenever the
	actor, which has one or more global actor references attached, gets executed.
	...
	it is, only global actors that are defined below the \textit{Flow} actor can be
	used (kind of ``super global actors'').
	\item The global actor should generate the correct type for the option it is
...
Similar to attaching global actors, storage values can be attached like
...
\begin{tight_itemize}
	...
	global actor based on the current value of the specified variable.
