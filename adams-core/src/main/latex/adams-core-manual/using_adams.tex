...
This time, you have to search for \textit{Display}. As soon as you have
...
\end{tight_itemize}
There are basically two reasons for sequential execution:
\begin{enumerate}
...
\begin{tight_itemize}
  ...
  \item \textit{CollectionSize} -- outputs the number of elements stored in a collection
	...
	into an array of is primitive counterparts (int or double).
	\item \textit{PrimitiveArrayToObjectArray} -- generates an array of objects
	from an array of primitives (Integer or Double).
...
\begin{tight_itemize}
	\item \textit{AbsolutePath} -- generates an absolute path from the incoming path string.
...
\begin{tight_itemize}
	...
	between two files.
	\item \textit{DirChanged} -- allows the monitoring of a directory with
	the specified directory change monitor.
...
\begin{tight_itemize}
	...
	\item \textit{GetMapKeyValuePairs} -- returns all key/value pairs as Object arrays of length 2.
...
At times, it can be important to determine the time it takes for certain actors to
be executed. Whether it be for finding bottlenecks or reporting performance figures.
ADAMS has the notion of \textit{timed actors}, that record the wall time it takes
for the managed actors to be executed. The following actors are available:
\begin{tight_itemize}
    \item \textit{TimedStandalone}
    \item \textit{TimedSource} -- \textit{adams-core-timedsource.flow}
    \item \textit{TimedSubProcess} -- \textit{adams-core-timedsubprocess.flow}
    \item \textit{TimedTee} -- \textit{adams-core-timedtee.flow}
    \item \textit{TimedTrigger} -- \textit{adams-core-timedtrigger.flow}
\end{tight_itemize}
...
Server via the \textit{adams-db} module). The following actors allow you to perform some basic operations:
...
\noindent The following sources are available:
\begin{tight_itemize}
  \item \textit{DatabaseConnectionValid} -- outputs \texttt{true} if the database connection is still valid.
...
module. On more information on JDBC URLs and databases, see \ref{using_adams_database_access},
\ref{extending_adams_database_access} and the manual for the \textit{adams-db} module.

The \textit{Table.props} configuration file contains various parameters for
tweaking database access, e.g.:
\begin{tight_itemize}
    \item \texttt{InitTables.Debug} -- outputs a stacktrace in case the initTable call for table fails (false|true)
    \item \texttt{AnsiQuotes} -- whether to use ANSI quotes, i.e., double quotes, around table and column names
    \item \texttt{TablePrefix} -- whether to prefix the table names with this string
    \item \texttt{TABLE\_CLASSNAME.Debug} -- whether debugging information is enabled for \texttt{TABLE\_CLASSNAME}
\end{tight_itemize}
...
The \textit{TriggerCallableStandalone} standalone actor can be used for triggering
standalones. If the callable actor is expecting input, then a null token is sent.
Of course, this will only work with callable actors that do not actually process
the data, like an instance of \textit{Trigger}.

The \textit{ListCallableActors} source allows you to list all the available callable
actors available in the scope of this listing actor. Useful when iterating through
a number of predefined setups.
...
common sub-flow, encapsulated by a \textit{Trigger} control actor, is
prompting the user for input parameters. The \textit{PromptUser} template inserts such a
sub-flow, consisting of a \textit{EnterManyValues} source and a \textit{MapToVariables}
transformer, enclosed by a \textit{Trigger}. When ticking \textit{restoreEnabled}, you
can also let the flow remember the parameters that the user entered the last time round.
Figures \ref{template-static_use1} to \ref{template-static_use3} show the use of this mechanism.
...
\begin{figure}[htb]
  ...
  \caption{The options of the \textit{PromptUser} template.}
...
\begin{tight_itemize}
	\item \textit{LoadVariables} -- standalone actor for loading variables, e.g.,
	...
	\item \textit{DeleteManyVariables} -- removes multiple variables in one go (standalone/transformer).
...
\heading{Overview of actors}
The following actors are available to handle storage values:
\begin{tight_itemize}
	...
	\item \textit{DeleteManyStorageValues} -- removes multiple storage values
	from internal memory (standalone/transformer), freeing up memory.
...
\begin{tight_itemize}
	...
	resume the flow execution \icon{images/resume}, step through it \icon{images/step},
	actor by actor, stop the flow \icon{images/stop_blue} or simply disable
	the current breakpoint \icon{images/debug_off}.
	\item It is possible to update the breakpoint condition whenever the breakpoint
	is reached.
	\item In the \textit{second} group you can view the source code \icon{images/source}
	...
	expressions \icon{images/glasses} (variables, boolean and numeric expressions; Figure
	\ref{floweditor-debugging1_controlpanel}), display an overview of all the
	variables \icon{images/variable} and their current values, inspect the current storage
	items \icon{images/storage}, you can inspect the current token \icon{images/properties}
	that is being passed through the breakpoint (Figure \ref{floweditor-debugging1_inspectionpanel})
	and also all breakpoints \icon{images/breakpoints} currently present in the flow (Figure
...
Stepping through a flow, using the \textit{Step} button \icon{images/step}
...
If you want to view flow files, you can do that with basically any text editor, as they are just
plain text files. However, there is basic syntax highlighting available for the following editors:
\begin{tight_itemize}
    \item vim \\
    \url{https://github.com/waikato-datamining/adams-base/tree/master/adams-core/src/site/resources/vim}{}
    \item Notepad++ \\
    \url{https://github.com/waikato-datamining/adams-base/tree/master/adams-core/src/site/resources/notepadpp}{}
\end{tight_itemize}
...
Since executing commands like this is a bit low-level, there is also a
...
\begin{tight_itemize}
  ...
  \item Download, extract and copy an Oracle JDK to \texttt{/opt/jdk}\footnote{If you
  prefer using the JDK managed through your Linux distro, then you can use
  \texttt{\$(readlink -f /usr/bin/javac | sed "s:/bin/javac::")} to determine
  the value for \texttt{JAVA\_HOME}.}
...
User=adams
Group=adams
Environment="JAVA_HOME=/opt/jdk" "PATH=/opt/jdk/bin:/usr/local/bin:/usr/bin"
...
; the following is necessary since daemon.sh exits and ExecStop is
...
\clearpage
\subsection{Directory layout for processing files}
Use the directory layout specified in \ref{example_linux_dir_layout_data_processing},
if your flow is processing files.
\begin{figure}[htb]
\dirtree{%
 .1 /.
 .2 opt.
...
ADAMS can be run as background process on Windows as well. There are two options,
which are described in the following sub-sections: NSSM\cite{nssm}, WinSW\cite{winsw}
or Task Scheduler\cite{taskschd}.

%%%%%%%%
% NSSM %
%%%%%%%%
\subsection{NSSM}
Using the NSSM\cite{nssm} tool, you can install a Windows service that executes the flow \textit{myflow.flow}.
...
\begin{tight_itemize}
  ...
  This user must have appropriate access to the adams directory.
...
%%%%%%%%%
\subsection{WinSW}
Using the WinSW\cite{winsw} tool, you can install a Windows service that executes the flow \textit{myflow.flow}.
ADAMS is assumed for be installed in \textit{C:\textbackslash adams} for this
example and \textit{myflow.flow} resides in the \textit{C:\textbackslash flows}
directory. The JDK is available from \textit{C:\textbackslash java\textbackslash jdk21}.
...
\begin{tight_itemize}
  ...
  don't want to run ADAMS with admin rights in order to avoid potential attacks.
  This user must have appropriate access to the adams directory.
  \item Download the WinSW x64 binary that includes the .NET runtime, to avoid having
  to install that as well (at the time of writing, this refers to version
  2.12.0\footnote{\url{https://github.com/winsw/winsw/releases/download/v2.12.0/WinSW-x64.exe}{}}).
  \item Place the WinSW binary in the \textit{C:\textbackslash flows} directory and rename it
  to \textit{myflow.exe}.
  \item Create an XML file called \textit{myflow.xml} alongside the binary with the following content:
  {\scriptsize
  \begin{verbatim}
<service>
  <id>myflow</id>
  <name>myflow</name>
  <description>Starts the ADAMS myflow workflow.</description>
  <env name="JAVA_HOME" value="C:\java\jdk21"/>
  <env name="BASEDIR" value="C:\adams"/>
  <workingdirectory>%BASEDIR%</workingdirectory>
  <executable>%JAVA_HOME%\bin\java</executable>
  <arguments>
   -classpath "%BASEDIR%\resources";"%BASEDIR%\lib\java-cup-11b-20160615.jar";"%BASEDIR%\lib\*"
   -Xmx4g
   ...
   adams.flow.FlowRunner
   -input "%BASE%\myflow.flow"
   -remote-scripting-engine-cmdline "adams.scripting.engine.DefaultScriptingEngine -port 12345"
  ...
  \end{verbatim}}
  \item You can then install the service using \textit{myflow.exe install}.
  \item You can then start the service using \textit{myflow.exe start}.
\end{tight_itemize}
\textbf{NB:} Whenever you make a change, you need to \textit{stop}, \textit{uninstall},
\textit{install} and \textit{start} the service, before the changes will take effect.
...
Using the Task Scheduler\cite{taskschd} tool, you can run a task that executes the flow \textit{myflow.flow}.
ADAMS is assumed for be installed in \textit{C:\textbackslash adams} for this
example and \textit{myflow.flow} resides in the \textit{C:\textbackslash flows}
directory.

\begin{tight_itemize}
    \item Launch the \textit{Task Scheduler}\footnote{How to launch the Task Scheduler: \url{https://windowsloop.com/how-to-open-task-scheduler/}{}}
    \item Open the \textit{Task Scheduler Library} folder
    \item Create a new folder, e.g., \textit{ADAMS}
    \item Create a \textit{Basic task}
    \item For the \textit{Trigger} select \textit{When the computer starts}
    \item For the \textit{Action} select \textit{Program}
    \item As \textit{Program/script}, select the batch file \textit{daemon.bat}
    from the ADAMS \textit{bin} directory.
    \item Use the following \textit{Arguments} to execute the flow with 1GB of
      heap and the remote command scripting engine on port 12345:
        \begin{verbatim}
        start C:\textbackslash myflow.flow 1g 12345
        \end{verbatim}
    \item \textit{Finish} the task
    \item \textit{Edit} the task
    \item On the \textit{General} tab, switch to \textit{Run whether the user is logged on or not}
    and provide your password.
\end{tight_itemize}

\clearpage
\subsection{Directory layout for processing files}
Use the directory layout specified in \ref{example_windows_dir_layout_data_processing},
if your flow is processing files.
\begin{figure}[htb]
\dirtree{%
 .1 C:/.
...
The following menu items are only available if you switch to user mode
...
\heading{Logging}
\noindent Various applications have the \texttt{-logging-handler} option which allows you to specify the classname(s)
of the log handlers you want to use (package: \texttt{adams.core.logging}.
Alternatively, you can also use the \texttt{ADAMS\_LOGGING\_HANDLER} environment variable for that.
Both approaches support a blank-separated list of classnames for specifying multiple handlers.

When using \texttt{FileHandler} or \texttt{RotatingFileHandler} for managing the logging output, then the
\texttt{ADAMS\_LOGFILE\_PREFIX} environment variable can be used to prefix the files. This is useful
for separating the log files when running multiple ADAMS processes on the same machine (e.g., as
Linux daemon or Windows service).
...
\heading{Properties files}
For configurations managed by ADAMS and stored in properties files (\texttt{.props}),
...
\label{using_adams_database_access}
Here are JDBC URL templates for accessing various types of databases:
\begin{tight_itemize}
  \item \textit{MySQL} -- \texttt{jdbc:mysql://<HOST>:3306/<DATABASE>}
  \item \textit{MySQL} -- \texttt{jdbc:mysql://<HOST>:3306/<DATABASE>?sslMode=PREFERRED}
  \item \textit{PostgreSQL} -- \texttt{jdbc:postgresql://<HOST>:5432/<DATABASE>}
  \item \textit{H2} -- \texttt{jdbc:h2:mem:<DATABASE>;DB\_CLOSE\_DELAY=-1;NON\_KEYWORDS=VALUE}
  \item \textit{HSQLDB} -- \texttt{jdbc:hsqldb:hsql://<HOST>:9001}
  \item \textit{SQLite} -- \texttt{jdbc:sqlite:<PATH\_TO\_FILE>}
  \item \textit{MS SQL Server} -- \verb|jdbc:sqlserver://HOST:1433;databaseName=DATABASE;...|
\end{tight_itemize}

\heading{Notes}
\begin{tight_itemize}
    \item Older versions of MySQL allowed \texttt{'0000-00-00 00:00:00'} as default value for TIMESTAMP columns.
    If you encounter problems with exceptions due to invalid dates, append the following to your JDBC URL:
    \texttt{\&\&sessionVariables=sql\_mode='ALLOW\_INVALID\_DATES'}
    \item If you need to connect to a MySQL database that requires SSL connections, then please refer to
    section \ref{mysql_ssl_connections}.
    \item If you get \textit{Cannot connect: java.sql.SQLNonTransientConnectionException: Public Key Retrieval is not allowed}
    then add \texttt{\&\&allowPublicKeyRetrieval=true} the JDBC URL.
    \item For more information on MySQL connection parameters, see section \href{https://dev.mysql.com/doc/connector-j/en/connector-j-reference-configuration-properties.html}{Configuration properties}
    of the Connector/J documentation.
    \item MS SQL Server connections place user and password into the JDBC URL: \\
    \texttt{;user=USER;password=PW;}
    \item To turn off encryption for MS SQL Server connections, append the following: \\
    \texttt{encrypt=false;}
\end{tight_itemize}


\subsection{MySQL imports}
Importing database dumps can take a long time. However, you can influence the speed by
turning off the double write buffer in your \texttt{mysql.cnf} config
file (section \texttt{mysqld}):

{\scriptsize
\begin{verbatim}
[mysqld]
# for faster sql dump imports
# https://dev.mysql.com/doc/refman/9.3/en/innodb-parameters.html#sysvar_innodb_doublewrite
innodb_doublewrite=off
\end{verbatim}}



\subsection{MySQL SSL connections}
\label{mysql_ssl_connections}
These sections in the MySQL reference manual were used as basis for the instructions:
\begin{tight_itemize}
    \item \href{https://dev.mysql.com/doc/refman/8.4/en/using-encrypted-connections.html}{Configuring MySQL to Use Encrypted Connections}
    \item \href{https://dev.mysql.com/doc/refman/8.4/en/creating-ssl-rsa-files-using-mysql.html}{Creating SSL and RSA Certificates and Keys using MySQL}
\end{tight_itemize}
And this one from the documentation for the JDBC connector:
\begin{tight_itemize}
    \item \href{https://dev.mysql.com/doc/connector-j/en/connector-j-server-authentication.html}{Setting up Server Authentication}
\end{tight_itemize}

\subsubsection{Create certificates and trust store}
\noindent The instructions below are for a Linux server with the MySQL configuration stored in
\texttt{/etc/mysql/mysql.conf.d/mysqld.cnf}, data directory located at \texttt{/var/lib/mysql}
and a user called \texttt{adamsuser}.
\begin{tight_itemize}
    \item Start up the MySQL server with automatic SSL/RSA file generation. Once started up,
    the data directory should contain several files with a \texttt{.pem} extension.
    \item Edit the \texttt{mysqld.cnf} file, adding the following content:
    \begin{verbatim}
[mysqld]
...
ssl_key=/var/lib/mysql/client-key.pem
    \end{verbatim}
    \item As root user, create the truststore file in the \texttt{/var/lib/mysql} directory with this command
    (do not forget to adjust the password):
    \begin{verbatim}
keytool -importcert -alias MySQLCACert -file ca.pem \
  -keystore truststore -storepass mypassword
    \end{verbatim}
    \item Move the trust store to your ADAMS home directory and correct the ownership:
    \begin{verbatim}
mv truststore /home/adamsuser/.adams/
chown adamsuser:adamsuser /home/adamsuser/.adams/truststore
    \end{verbatim}
\end{tight_itemize}

\subsubsection{Make trust store available to ADAMS}
The \textit{adams-security} module is required if you want to reference the truststore
via the global \textit{Preferences} dialog. Otherwise, you have to modify the start script for
ADAMS to include the following JVM options (adjust the password appropriately):
\begin{verbatim}
-Djavax.net.ssl.trustStore=/home/adamsuser/.adams/truststore
-Djavax.net.ssl.trustStorePassword=mypassword
\end{verbatim}

\subsubsection{Enable SSL in database tool}
If you use database tools, like \href{https://dbeaver.io/}{DBeaver} for managing databases, then
you need to copy the CA certificate and the client-related files into your home directory to have access
to them as regular user:
\begin{tight_itemize}
    \item Create a directory for the files:
    \begin{verbatim}
mkdir /home/adamsuser/.ssh/mysql
    \end{verbatim}
    \item Change into the new directory:
    \begin{verbatim}
cd /home/adamsuser/.ssh/mysql
    \end{verbatim}
    \item Copy the relevant files:
    \begin{verbatim}
sudo cp /var/lib/mysql/ca.pem \
  ...
  .
    \end{verbatim}
    \item Fix the ownership:
    \begin{verbatim}
sudo chown adamsuser:adamsuser *.pem
    \end{verbatim}
    \item Edit the database connection in your tool: enable SSL and point to the CA and client
    certificates and the client private key.
\end{tight_itemize}


\section{Troubleshooting}
\begin{tight_itemize}
  \item When the Flow editor warns \textit{Flow was saved with additional modules:} after loading
  a flow, then this indicates that the current flow was saved with another ADAMS configuration.
  Depending on what actors are used in the flow, it may not be possible to load the flow.
  The list of modules gives a hint at the ADAMS application that the flow was written with.
  If these warnings are annoying, they can be turned off by creating the \texttt{Option.props}
  config file containing \texttt{option.ReportModuleDifferences=false} (see \ref{propsfiles}
  for more details where to store the file).
  ...
  \item The JVM crashes on Windows after disconnecting the session. Windows 11 and
  later seems to be more aggressive with freeing up resources. Adding
  \texttt{-Dsun.java2d.d3d=false} to your Java call helps avoiding this (ADAMS does
  not rely on any 3D libraries).

  \item You get the error \texttt{Failed to exec spawn helper} when launching
  external processes via the \textit{Exec} or \textit{Command} actors. Add the
  following option to your Java call (or to the \texttt{\_JAVA\_OPTS} environment
  variable)\footnote{Found here: \url{https://stackoverflow.com/a/61325264}{}}: \
  \begin{verbatim}
