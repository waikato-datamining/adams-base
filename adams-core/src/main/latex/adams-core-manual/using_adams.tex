...
/opt/jdk/bin/java -cp "./lib/*" adams.flow.FlowRunner -headless -input ./flows/myflow.flow &
  ...
  which will execute your flow at startup time:
    ...
    \end{verbatim}
  \item Instead of a cronjob, you can also use a systemd startup script
  \textit{adams.service} (using user/group adams/adams in this example):
...
ExecStart=/opt/jdk/bin/java -cp "./lib/*:./lib/linux64/*" adams.flow.FlowRunner -input "./flows/myflow.flow" -headless true -register true -non-interactive true
...
\subsection{Starting a daemon}
Below is a slightly more elaborate script for starting a daemon. It tells the
\textit{FlowRunner} to launch a scripting engine that listens on port 12345.
This allows you to remotely log in to a server, launch the ADAMS user interface
using an X session and communicate with the daemon flow using the Remote control
center.
{\scriptsize
\begin{verbatim}
BASEDIR=`dirname $0`/..
...
"$JCMD" \
  ...
  -Xmx1g \
  adams.flow.FlowRunner \
  -input "$BASEDIR/flows/myflow.flow" \
...
\subsection{Stopping a daemon}
For stopping a daemon flow properly instead of killing the process, you can
send the \textit{Stop} command using the following script:
{\scriptsize
\begin{verbatim}
BASEDIR=`dirname $0`/..
BASEDIR=`(cd "$BASEDIR"; pwd)`
...
"$JCMD" \
  ...
  -connection "adams.scripting.connection.DefaultConnection -host 127.0.0.1 -port 12345" \
...
tool, you can install the following batch file called \textit{myflow.bat} (e.g., in
directory \textit{C:\textbackslash adams\_services}) which executes the flow
\textit{myflow.flow} (ADAMS is installed in directory \textit{C:\textbackslash adams}):

{\scriptsize \begin{verbatim}
REM starts myflow
cd C:\\adams
javaw -cp ".\\lib\\*;.\\lib\\windows64\\*" adams.flow.FlowRunner^
 -input ".\\flows\\myflow.flow"^
 -headless true^
 -register true^
 -non-interactive true
\end{verbatim}}

\noindent Use the following steps to install the batch file as service:
\begin{tight_itemize}
  ...
  \textit{nssm.exe} executable is located):
    ...
    nssm install adams
    \end{verbatim}
  \item On the \textit{Application} tab, select \textit{myflow.bat} as the \textit{Path}.
...
Below is a slightly more elaborate batch file for starting a server. It tells the
\textit{FlowRunner} to launch a scripting engine that listens on port 12345.
This allows you to remotely log in to a server, launch the ADAMS user interface
via RDP and communicate with the service flow using the Remote control center.
{\scriptsize
\begin{verbatim}
@echo off
set JCMD=java
...
%JCMD% -classpath %CLASSPATH% -Xmx1g adams.flow.FlowRunner^
 -input "%BASEDIR%\flows\myflow.flow"^
 -remote-scripting-engine-cmdline^ "adams.scripting.engine.DefaultScriptingEngine -port 12345"^
 -headless true^
 -non-interactive true^
...
For stopping a daemon flow properly instead of killing the process, you can
send the \textit{Stop} command using the following batch file (the last command
is spread over three lines to fit on the page):
{\scriptsize
\begin{verbatim}
@echo off
set JCMD=java
...
set RESOURCES=%BASEDIR%\resources
set CLASSPATH="%RESOURCES%";"%REPO%\java-cup-11b-2015.03.26.jar";"%REPO%\*"
%JCMD% -classpath %CLASSPATH% adams.scripting.CommandRunner -start-local-engine false^
 -connection "adams.scripting.connection.DefaultConnection -host 127.0.0.1 -port 12345"^
 -command adams.scripting.command.basic.Stop
\end{verbatim}}
