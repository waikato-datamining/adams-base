...
/*
 * StratifyTest.java
 * Stratifyright (C) 2019 University of Waikato, Hamilton, NZ
 */
...
public class StratifyTest
   ...
   */
  public StratifyTest(String name) {
    super(name);
  }
  ...
  protected List<String> generateOutput() {
    ...
    for (i = 0; i < 3; i++) {
      for (n = 0; n < num; n++) {
        data.add(new Binnable<>(i*num + n, i));
      }
    }
    saveData(data, fname);
    result.add(fname);
    ...
    saveData(Stratify.stratify(data, 10), fname);
    result.add(fname);
  ...
  public static Test suite() {
    return new TestSuite(StratifyTest.class);
  }
