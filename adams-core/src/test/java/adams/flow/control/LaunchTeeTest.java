...
/**
 * Test for LaunchSequence actor.
 *
 * @author fracpete
 * @author adams.core.option.FlowJUnitTestProducer (code generator)
 */
public class LaunchTeeTest
   ...
   */
  public LaunchTeeTest(String name) {
    super(name);
  }
  ...
  public static Test suite() {
    return new TestSuite(LaunchTeeTest.class);
  }
  ...
  public AbstractActor getActor() {
    ...
    try {
      ...
      // Flow.LaunchSequence
      LaunchTee launchsequence = new LaunchTee();
      List<Actor> actors3 = new ArrayList<>();

      // Flow.LaunchSequence.MathExpression
      MathExpression mathexpression = new MathExpression();
      argOption = (AbstractArgumentOption) mathexpression.getOptionManager().findByProperty("expression");
      mathexpression.setExpression((MathematicalExpressionText) argOption.valueOf("@{i} * X * rand()"));
      actors3.add(mathexpression);

      // Flow.LaunchSequence.CallableSink
      ...
      launchsequence.setActors(actors3.toArray(new Actor[0]));

      argOption = (AbstractArgumentOption) launchsequence.getOptionManager().findByProperty("scopeHandlingVariables");
      launchsequence.setScopeHandlingVariables((ScopeHandling) argOption.valueOf("COPY"));
      actors.add(launchsequence);
