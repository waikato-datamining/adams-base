...
import adams.flow.transformer.StringArraySplit.SplitHandling;
...
/**
 * Test for StringArraySplit actor.
 *
 * @author fracpete
...
public class StringArraySplitTest
   ...
   */
  public StringArraySplitTest(String name) {
    super(name);
  }
  ...
  protected void setUp() throws Exception {
    ...
    m_TestHelper.copyResourceToTmp("small.log");
  ...
  protected void tearDown() throws Exception {
    m_TestHelper.deleteFileFromTmp("small.log");
    m_TestHelper.deleteFileFromTmp("dumpfile.txt");
  ...
  public static Test suite() {
    return new TestSuite(StringArraySplitTest.class);
  }
  ...
  public AbstractActor getActor() {
    ...
    try {
      ...
      files.add((PlaceholderFile) argOption.valueOf("${TMP}/small.log"));
      ...
      // Flow.StringArraySplit
      StringArraySplit stringarraysplit = new StringArraySplit();
      stringarraysplit.setOutputArray(true);

      argOption = (AbstractArgumentOption) stringarraysplit.getOptionManager().findByProperty("regExp");
      stringarraysplit.setRegExp((BaseRegExp) argOption.valueOf("^[0-9][0-9][0-9][0-9]-[0-9][0-9]-.*"));
      argOption = (AbstractArgumentOption) stringarraysplit.getOptionManager().findByProperty("splitHandling");
      stringarraysplit.setSplitHandling((SplitHandling) argOption.valueOf("AT_START"));
      actors.add(stringarraysplit);

      // Flow.ArrayProcess
      ArrayProcess arrayprocess = new ArrayProcess();
      List<Actor> actors2 = new ArrayList<>();

      // Flow.ArrayProcess.StringJoin
