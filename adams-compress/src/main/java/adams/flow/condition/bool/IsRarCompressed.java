...
public class IsRarCompressed
  ...
  public String globalInfo() {
    return "Checks whether the file or byte array is Rar compressed.";
  }
  ...
  protected boolean doEvaluate(Actor owner, Token token) {
    if (token.hasPayload(String.class))
      return RarUtils.isRarCompressed(new PlaceholderFile(token.getPayload(String.class)));
    else if (token.hasPayload(File.class))
      return RarUtils.isRarCompressed(token.getPayload(File.class));
    else if (token.hasPayload(byte[].class))
      return RarUtils.isRarCompressed(token.getPayload(byte[].class));
    else
      return false;
  }
}
