...
public class IsXzCompressed
  ...
  protected boolean doEvaluate(Actor owner, Token token) {
    if (token.hasPayload(String.class))
      return XzUtils.isXzCompressed(new PlaceholderFile(token.getPayload(String.class)));
    else if (token.hasPayload(File.class))
      return XzUtils.isXzCompressed(token.getPayload(File.class));
    else if (token.hasPayload(byte[].class))
      return XzUtils.isXzCompressed(token.getPayload(byte[].class));
    else
      return false;
  }
}
