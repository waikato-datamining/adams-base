...
public class IsZipCompressed
  ...
  public String globalInfo() {
    return "Checks whether the file or byte array is Zip compressed.";
  }
  ...
  protected boolean doEvaluate(Actor owner, Token token) {
    if (token.hasPayload(String.class))
      return ZipUtils.isZipCompressed(new PlaceholderFile(token.getPayload(String.class)));
    else if (token.hasPayload(File.class))
      return ZipUtils.isZipCompressed(token.getPayload(File.class));
    else if (token.hasPayload(byte[].class))
      return ZipUtils.isZipCompressed(token.getPayload(byte[].class));
    else
      return false;
  }
}
