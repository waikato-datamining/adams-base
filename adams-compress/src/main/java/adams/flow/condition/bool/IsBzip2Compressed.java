 ...
 */
public class IsBzip2Compressed
  ...
  public String globalInfo() {
    return "Checks whether the file or byte array is Bzip2 compressed.";
  }
  ...
  protected boolean doEvaluate(Actor owner, Token token) {
    if (token.hasPayload(String.class))
      return Bzip2Utils.isBzip2Compressed(new PlaceholderFile(token.getPayload(String.class)));
    else if (token.hasPayload(File.class))
      return Bzip2Utils.isBzip2Compressed(token.getPayload(File.class));
    else if (token.hasPayload(byte[].class))
      return Bzip2Utils.isBzip2Compressed(token.getPayload(byte[].class));
    else
      return false;
  }
}
