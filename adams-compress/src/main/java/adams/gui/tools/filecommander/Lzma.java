...
public class Lzma
  ...
  protected void doActionPerformed(ActionEvent e) {
	...
	String msg = LzmaUtils.compress(file, 1024);
	if (msg != null) {
	  GUIHelper.showErrorMessage(getOwner(), "Failed to compress the following file using LZMA:\n" + file + "\n" + msg);
	}
	else {
	  getOwner().getActive().reload();
	  showStatus("Compressed " + file + " using LZMA");
	}
	return null;
      }
    };
  ...
  public void update() {
    ...
    setEnabled(local && (files.length == 1) && files[0].isFile() && !files[0].getName().endsWith(LzmaUtils.EXTENSION));
  }
}
