...
public class CannyEdgeDetection
  ...
  protected BoofCVImageContainer[] doTransform(BoofCVImageContainer img) {
    ...
    gray      = (GrayU8) BoofCVHelper.toBoofCVImage(img.getImage(), BoofCVImageType.GRAYU8);
    edgeImage = new GrayU8(gray.width, gray.height);

    // Create a canny edge detector which will dynamically compute the threshold based on maximum edge intensity
    // It has also been configured to save the trace as a graph.  This is the graph created while performing
    // hysteresis thresholding.
    canny = FactoryEdgeDetectors.canny(m_BlurRadius, true, true, GrayU8.class, GrayS16.class);
